...
Symbit is a MoonBit symbolic mathematics library with a broad SymPy-aligned surface. The project is not a thin wrapper over Python: the implementation lives in MoonBit packages under `src/sym*`, while the `src/sympy/*` packages are reserved for oracle parity tests and never participate in runtime semantics. In practice this means Symbit is meant to be linked into MoonBit programs as a native symbolic engine: expressions, simplifiers, polynomial domains, statistics, physics, geometry, logic, combinatorics, and related algebraic subsystems are all represented as MoonBit values and manipulated locally.
